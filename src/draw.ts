...
export function start(state: State, e: cg.MouchEvent): void {
  ...
  const orig = getKeyAtDomPos(position, state.orientation === 'white', state.dom.bounds(), state.boardType);
  if (!orig) return;
...
export function processDraw(state: State): void {
  raf(() => {
    ...
    if (cur) {
      const mouseSq = getKeyAtDomPos(cur.pos, state.orientation === 'white', state.dom.bounds(), state.boardType);
